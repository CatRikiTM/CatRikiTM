@app.on_message(filters.command(["cas"], prefixes="."))
async def skkalalwo(Client, message):
	path = "cat.jpg"
	await app.download_media(message.reply_to_message.photo, file_name=path, block=True)
	im = Image.open('downloads/cat.jpg')
	...
	im.save('saved/hui.jpg')
	im.show()
	await app.send_photo(message.chat.id, 'saved/hui.jpg')
